...
대용량 메세지를 보내는 방법
카프카에 토픽당 하나의 메세지는 1MB가 default인데 더 큰 용량은 카프카에서 비효율적이고 안티패턴으로 여겨진다. 
그럼에도 불구하고 대용량 메세지를 보내야한다면 두가지 방법이 존재하는데  
1. 외부 스토리지 사용 : S3에 해당 메세지를 올리고 해당 메세지의 레퍼런스를 카프카로 보내는 방법이다.
ex) 비디오, 파일을 보내는데 힘들다면 S3에 업로드하고 소용량 메타데이터인 레퍼런스만 카프카로 전송하고 S3로부터 consume 내려받기
2. 카프카 파라미터 매개변수 수정 : 브로커, 프로듀서, 컨슈머 세팅 바꾸기
ex 각 파트 max byte를 최대 복제 크기를 10MB로 수정할 수 있다. 
